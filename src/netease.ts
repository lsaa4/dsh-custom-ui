...
let proxyAgentCache: { url: string; agent: HttpAgent } | null = null

function readProxy(): string {
  try {
    const home = process.env.DSH_HOME ?? join(homedir(), '.dsh')
    const raw = JSON.parse(readFileSync(join(home, 'data', 'glass-ui', 'config.json'), 'utf8')) as {
      neteaseProxy?: unknown
    }
...
function agentFor(proxyUrl: string): HttpAgent | undefined {
...
function randomSecret(): string {
  ...
  let out = ''
  for (let i = 0; i < 16; i++) out += chars[randomBytes(1)[0]! % chars.length]
  return out
}

function weapiParams(data: unknown): { params: string; encSecKey: string } {
...
function readCookie(): string {
  try {
    const raw = JSON.parse(readFileSync(cookiePath(), 'utf8')) as { cookie?: string }
    return raw.cookie ?? ''
  } catch {
    return ''
  }
...
function mergeCookies(saved: string, setCookie: readonly string[] | undefined): string {
  if (setCookie === undefined || setCookie.length === 0) return saved
  ...
  for (const raw of setCookie) {
    ...
    if (name !== undefined) map.set(name, first)
  }
...
async function remoteJson(url: string, headers: Record<string, string>): Promise<Record<string, unknown>> {
  ...
  if (text === '') throw new Error(`remote api ${url} returned empty response`)
  return JSON.parse(text) as Record<string, unknown>
}

/** Adapt one weapi-style call to a NeteaseCloudMusicApi-compatible server. */
async function remoteWeapiRequest(
  apiBase: string,
  path: string,
  data: Record<string, unknown>,
  cookie: string,
): Promise<{ json: Record<string, unknown>; setCookie: string[] }> {
  ...
  switch (path) {
    case '/weapi/login/qrcode/unikey': {
      ...
      if (key !== '') {
        try {
          const img = await remoteJson(
            `${apiBase}/login/qr/create?key=${encodeURIComponent(key)}&qrimg=true`,
            headers,
          )
    ...
    case '/weapi/user/playlist': {
      ...
      const j = await remoteJson(
        `${apiBase}/user/playlist?uid=${uid}&limit=${limit}&offset=${offset}`,
        headers,
      )
    ...
    case '/weapi/v1/discovery/recommend/songs': {
      ...
      const dailySongs = (j.data as { dailySongs?: unknown } | undefined)?.dailySongs ?? []
    ...
    case '/weapi/song/enhance/player/url': {
      ...
      const first = (j.data as Array<{ url?: string | null }> | undefined)?.[0]
...
async function weapiRequest(path: string, data: unknown, cookie: string): Promise<{
  json: unknown
  setCookie: string[]
}> {
...
async function publicLyric(id: string): Promise<{ lrc?: string }> {
  const apiBase = readApiBase()
  const proxy = readProxy()
  const agent = agentFor(proxy)
  const url = apiBase !== ''
  ...
  const json = await res.json().catch(() => ({})) as { lrc?: { lyric?: string } }
...
function readSmallBody(req: IncomingMessage, max = 128 * 1024): Promise<Buffer> {
...
export function registerNeteaseRoutes(host: NeteaseRoutesHost): Array<() => void> {
  ...
  disposers.push(host.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const body = JSON.parse((await readSmallBody(req)).toString('utf8')) as {
          path?: string
          data?: Record<string, unknown>
        }
  ...
  disposers.push(host.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const json = apiBase !== ''
          ? await remoteJson(
              `${apiBase}/search?keywords=${encodeURIComponent(q)}&limit=20`,
              readCookie() !== '' ? { cookie: readCookie() } : {},
            ) as {
              result?: { songs?: Array<{
                id?: number
          ...
          : await publicJson(
              `/api/search/get?type=1&s=${encodeURIComponent(q)}&limit=20`,
            ) as {
              result?: { songs?: Array<{
                id?: number
                name?: string
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (apiBase !== '') {
          ...
          const j = await remoteJson(
            `${apiBase}/song/url?id=${encodeURIComponent(id)}`,
            readCookie() !== '' ? { cookie: readCookie() } : {},
          )
          const first = (j.data as Array<{ url?: string | null }> | undefined)?.[0]
        ...
        const redirect = await fetch(
          ...
          { redirect: 'manual', ...(agent !== undefined ? { agent } : {}) },
        )
  ...
  disposers.push(host.register({
    ...
    handler: async (_req, res) => {
      ...
      try {
        ...
        const profile = (json as { profile?: { nickname?: string; avatarUrl?: string; userId?: number } })
          .profile
  ...
  disposers.push(host.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const body = JSON.parse((await readSmallBody(req)).toString('utf8')) as { cookie?: string }
        ...
        writeCookie(cookie)
        const { json } = await weapiRequest('/weapi/w/nuser/account/get', {}, cookie)
        const profile = (json as { profile?: { nickname?: string; userId?: number } }).profile
        if (profile === undefined) {
          clearCookie()
